...
storage_status_t _storage_write(uint32_t page_addr, uint8_t* buff, uint32_t len, storage_header_status_appointment_t appointment)
{
	...
	uint32_t cur_len        = 0;
	storage_status_t status = FLASH_OK;
	...
	while (cur_len < len) {
	    ...
	    if ((payload.header.status & STORAGE_HEADER_STATUS_APPOINTMENT_COMMON) && (flash_status == FLASH_ERROR)) {
#if STORAGE_DEBUG
		...
		memset((uint8_t*)&payload, 0, sizeof(payload));
		status = _storage_read(page_addr, (uint8_t*)&payload, sizeof(payload), appointment);
...
storage_status_t _storage_read(uint32_t page_addr, uint8_t* buff, uint32_t len, storage_header_status_appointment_t appointment)
{
    ...
    while (cur_len < len) {
        ...
        status = flash_w25qxx_read(cur_addr, (uint8_t*)&payload, sizeof(payload));
...
#if STORAGE_DEBUG
			LOG_TAG_BEDUG(STORAGE_TAG, "read page (address=%lu): bad storage version %i!=%i", cur_addr, payload.header.version, STORAGE_PAYLOAD_VERSION);
#endif
		...
		if (!(payload.header.status & appointment)) {
#if STORAGE_DEBUG
		...
		if (search_start && !(payload.header.status & STORAGE_HEADER_STATUS_START)) {
#if STORAGE_DEBUG
		...
		if (!payload.header.next_addr && !(payload.header.status & STORAGE_HEADER_STATUS_END)) {
#if STORAGE_DEBUG
    ...
    if (!(payload.header.status & STORAGE_HEADER_STATUS_END)) {
#if STORAGE_DEBUG
...
#if STORAGE_DEBUG
			LOG_TAG_BEDUG(STORAGE_TAG, "read errors list page=%lu: check appointment error", page_addr);
#endif
			break;
		}
...
#if STORAGE_DEBUG
			LOG_TAG_BEDUG(STORAGE_TAG, "read errors list page=%lu: found last errors list", page_addr, status);
#endif
...
uint32_t _storage_get_size()
{
	return flash_w25qxx_get_pages_count() * STORAGE_PAGE_SIZE;
}

uint32_t _storage_get_errors_list_pages_count()
{
	uint32_t storage_pages_count = flash_w25qxx_get_pages_count();
